function [slbp3D,slbp2D]=SLBP_max(img,lbpRadius,lbpPoints,lbpRadiusPre,mapping)
...
if lbpRadius == 2
    ...
    imgMax=max(imgblks,[],1);
    imgCurr = reshape(imgMax,size(img));
...
%     imgCurr = reshape(imgMedian,size(img));
    ...
    imgMax=max(imgblks,[],1);
    imgPre = reshape(imgMax,size(img));
%     imgMedian = median(imgblks);
