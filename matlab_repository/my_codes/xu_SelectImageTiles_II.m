...
% from the authors.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%# Usage: only consider the large WSI in the glass, only choose 30 tiles from the WSI by using pp,ppt
function [top_left,bottom_right]=xu_SelectImageTiles_II(bwTissue,bwHimg,pp,ppt,tileSize,augmentStep)
...
for j=1:length(step)
    ...
    if sum(sum(Bt))>mnum                                 %% make sure the number is not too large
        temp=sort(B(Bt),'descend');
        Bt=(B.*Bt>temp(mnum));
    end
