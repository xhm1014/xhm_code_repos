function [slbp3D,slbp2D]=SLBP_min(img,lbpRadius,lbpPoints,lbpRadiusPre,mapping)
...
if lbpRadius == 2
    ...
    imgMin=min(imgblks,[],1);
    imgCurr = reshape(imgMin,size(img));
...
%     imgCurr = reshape(imgMedian,size(img));
    ...
    imgMin=min(imgblks,[],1);
    imgPre = reshape(imgMin,size(img));
%     imgMedian = median(imgblks);
