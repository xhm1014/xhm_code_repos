function blocks = cirInterpSingleRadius_SLBP(img,lbpPoints,lbpRadius,maxRadius)
...
imgNewH = imgH - 2*maxRadius;
imgNewW = imgW - 2*maxRadius;

% the interpolated img
dd=maxRadius-lbpRadius; %add by Hongming
...
for i = 1 : neighbors
    ...
    if (abs(x - rx) < 1e-6) && (abs(y - ry) < 1e-6)
        ...
        %blocks(i,:) = imgNew(:)'; % BY HONGMING
        imgNew2=imgNew(dd+1:end-dd,dd+1:end-dd);
        blocks(i,:) = imgNew2(:)';
    else
