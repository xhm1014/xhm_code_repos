function [slbp3D,slbp2D]=SLBP_std(img,lbpRadius,lbpPoints,lbpRadiusPre,mapping)
...
if lbpRadius == 2
    ...
    imgStd=std(imgblks,0,1);
    imgCurr = reshape(imgStd,size(img));
...
%     imgCurr = reshape(imgMedian,size(img));
    ...
    imgStd=std(imgblks,0,1);
    imgPre = reshape(imgStd,size(img));
%     imgMedian = median(imgblks);
